...
PAPER_ID_PATTERN = re.compile(r"\[([A-Z]\d+-\d+)\]")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Root containing processed PDF files whose names begin with citation tags like [A2-104] or [C1-19] (pattern: ^\\[[A-Z]\\d+-\\d+\\]).",
    )
    parser.add_argument(
        "--worktree-root",
        type=Path,
