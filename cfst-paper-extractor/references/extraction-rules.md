...
A paper is `is_valid=true` when the workflow can keep at least one extractable CFST specimen in the extraction universe and all are true:

- research object includes CFST columns or stub columns
- paper contains physical specimen test evidence
- at least one kept CFST specimen has usable specimen-level experimental capacity data
- loading mode is axial compression, eccentric compression, or a clearly separable mixture of those two modes

A paper may remain `is_valid=true` even when every kept CFST specimen is non-ordinary and is ultimately represented only through `excluded_specimens`. Ordinary-dataset inclusion is decided later at the specimen level and must not be used to invalidate an otherwise extractable paper.
...
The ordinary gate uses specimen-level evaluation over the kept CFST specimen universe. Evaluate every kept CFST specimen individually as ordinary or non-ordinary before final JSON assembly. In schema-v2.2, ordinary CFST specimens stay as full rows in `Group_A`, `Group_B`, or `Group_C`; non-ordinary CFST specimens are represented through grouped top-level `excluded_specimens` bundles. The paper-level `is_ordinary_cfst` is derived: `true` when the paper contains at least one ordinary specimen row kept in `Group_A` / `Group_B` / `Group_C`, `false` otherwise.
...
- `concrete_type` records the primary concrete family: `normal`, `high_strength`, `recycled`, `lightweight`, `self_consolidating`, `alkali_activated`, `geopolymer`, `expansive`, `uhpc`, `other`, `unknown`.
- `material_modifiers` records additional subtype tags, secondary material refinements, admixture-driven functions, or remaining non-ordinary material systems as a list of strings (empty list `[]` for plain concrete). It is not a fully closed vocabulary: use the canonical tags below whenever defensible, but do not reject an otherwise valid ordinary-compatible family solely because the paper uses a novel subtype phrase that still maps clearly at the family level.
...
- alkali-activated family tags: `alkali_activated` (legacy alias), `alkali_activated_slag`, `alkali_activated_fly_ash`, `alkali_activated_metakaolin`, `alkali_activated_calcined_clay`, `alkali_activated_natural_pozzolan`, `alkali_activated_blend`, `alkali_activated_hybrid`
- geopolymer family tags: `geopolymer` (legacy alias), `fly_ash_geopolymer`, `slag_geopolymer`, `metakaolin_geopolymer`, `calcined_clay_geopolymer`, `natural_pozzolan_geopolymer`, `blended_geopolymer`
...
- lightweight aggregate concrete with carbon-steel tube and no modifiers
- self-consolidating concrete with carbon-steel tube and no modifiers
- recycled aggregate concrete with explicit `R%`, carbon-steel tube, and no modifiers
- alkali-activated concrete with carbon-steel tube and only ordinary-compatible alkali-activated family tags
- geopolymer concrete with carbon-steel tube and only ordinary-compatible geopolymer family tags
- expansive / shrinkage-compensating / self-stressing concrete with carbon-steel tube and only ordinary-compatible expansive family tags
- static monotonic axial or eccentric compression
...
6. For mixed papers (some rows plain, some modified), classify specimen by specimen — not at paper level.

Treat the normalized high-risk aliases above as equivalent to their canonical non-ordinary tags when applying the ordinary gate and when populating `ordinary_exclusion_reasons`.
...
When a kept CFST specimen fails the specimen-level ordinary screen, mark it non-ordinary and record each failing condition in `ordinary_exclusion_reasons`. Common reasons:
...
`material_modifiers`: a list of strings (may be empty). Use it for both ordinary-compatible family refinement tags and remaining non-ordinary modifiers.

Ordinary-compatible family refinement tags include:

- alkali-activated family: `alkali_activated`, `alkali_activated_slag`, `alkali_activated_fly_ash`, `alkali_activated_metakaolin`, `alkali_activated_calcined_clay`, `alkali_activated_natural_pozzolan`, `alkali_activated_blend`, `alkali_activated_hybrid`
- geopolymer family: `geopolymer`, `fly_ash_geopolymer`, `slag_geopolymer`, `metakaolin_geopolymer`, `calcined_clay_geopolymer`, `natural_pozzolan_geopolymer`, `blended_geopolymer`
- expansive family: `expansive_concrete`, `shrinkage_compensating_concrete`, `self_stressing_concrete`, `type_k_expansive`, `type_m_expansive`, `type_s_expansive`, `calcium_sulfoaluminate_expansive`, `cao_expansive`, `mgo_expansive`, `composite_expansive`

Remaining non-ordinary modifier values:
...
- `material_modifiers`: list of additional concrete subtype / modification tags; use empty list `[]` for plain ordinary concrete; never `null`; any remaining non-ordinary modifier (see section 2.2) makes the specimen non-ordinary regardless of `concrete_type`
...
- `material_modifiers` contains no remaining non-ordinary modifier (see section 2.2)
- `loading_pattern = monotonic`
...
If paper is outside the experimental CFST-column scope, or no extractable kept CFST specimens remain:
