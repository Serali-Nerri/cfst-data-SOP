...
Recommended parent scheduling loop:

- Keep an in-memory active-worker map keyed by `paper_id`.
- While there are still effective `prepared` jobs or active workers, first top up the active set until it reaches the concurrency cap of 5.
- For each new launch, create the worktree, spawn exactly one worker with `fork_context=false`, then immediately mark that paper `running`.
- After the active set is full, or when there are no more `prepared` jobs to top up, perform one long poll over the active workers for up to 600000 ms (10 minutes).
- If the long poll returns no terminal status, treat that as normal ongoing work. Do not interrupt, replace, or redirect any worker just because the wait timed out.
- When one or more workers reach terminal status, process each result fully before the next poll: verify temp output presence, update `batch_state.json`, clean up the worktree, close the finished sub-agent, and only then top up the queue again.
...
When polling worker sub-agents from the parent, use a 10-minute wait by default:

- preferred poll interval: `timeout_ms=600000`
- shorter waits are for explicit diagnostics only, not the normal orchestration loop
...
- Spawn every worker sub-agent with `fork_context=false`.
...
- In the normal parent loop, use 10-minute polling (`timeout_ms=600000`) over the active worker set instead of short periodic waits.
- After any worker reaches terminal status, finish its full parent-side bookkeeping before spawning replacements: verify temp output, update state, remove worktree, close the worker, then refill open concurrency slots.
