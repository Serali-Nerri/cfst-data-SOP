...
def read_csv_specimens(csv_path: Path) -> tuple[list[str], list[str]]:
...
def read_recommended_rows(csv_path: Path) -> tuple[list[str], list[list[str]]]:
    with csv_path.open(encoding="utf-8") as handle:
        reader = csv.reader(handle, delimiter="\t")
        try:
            header = next(reader)
...
def main() -> int:
    ...
    try:
        csv_header, specimens = read_csv_specimens(args.csv)
    ...
    if change_count is not None:
        if change_count > 0:
            if not args.recommended_csv.is_file():
                ...
                rec_header, rec_rows = read_recommended_rows(args.recommended_csv)
