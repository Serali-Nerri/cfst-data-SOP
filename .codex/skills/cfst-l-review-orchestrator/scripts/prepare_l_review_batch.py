#!/usr/bin/env python3
"""Prepare a CFST L-review batch workspace from a Pending L-review root."""
...
def select_package(
    ...
    if not exact:
        ...
        return long_dirs[0], "unprepared_package", [
            ...
            f"prepare_rawdata_package_first:{long_dirs[0]}",
        ]
    ...
    *,
    ...
    for paper_id in paper_ids:
        ...
        if package_dir and selection_status == "candidate":
            package_info = inspect_l_review_package(package_dir, paper_id)
            status = PACKAGE_READY_STATUS if package_info["ready"] else "invalid_package"
            issues.extend(package_info["issues"])
        validate_status(status)

        tmp_review = output_root / "tmp" / paper_id / "review.md"
        tmp_recommended_csv = output_root / "tmp" / paper_id / f"{paper_id}_recommended.csv"
        final_review = output_root / "output" / "L-review" / paper_id / "review.md"
        final_recommended_csv = output_root / "output" / "L-review" / paper_id / f"{paper_id}_recommended.csv"

        batch_entry = {
            "paper_id": paper_id,
            "citation_tag": f"[{paper_id}]",
            "package_dir": str(package_dir) if package_dir else None,
            "worker_review_path": str(tmp_review),
            "worker_recommended_csv_path": str(tmp_recommended_csv),
            "final_review_path": str(final_review),
            "final_recommended_csv_path": str(final_recommended_csv),
            "status": status,
            "issues": issues,
        ...
        worker_job = {
            ...
            "extracted_csv_path": package_info.get("extracted_csv_path"),
            "worker_review_path": str(tmp_review),
            "worker_recommended_csv_path": str(tmp_recommended_csv),
            "final_review_path": str(final_review),
            "final_recommended_csv_path": str(final_recommended_csv),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare CFST L-review batch workspace from a Pending root.")
    parser.add_argument(
        "--pending-root",
        type=Path,
        required=True,
        help="Root containing pre-extracted L-review packages such as [A1-1]/.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    parser.add_argument(
        "--reset-state",
        action="store_true",
        help="Overwrite existing l_review_batch_state.json instead of preserving operational state by paper_id.",
    )
...
def main() -> int:
    ...
    final_output_dir = output_root / "output" / "L-review"
    ...
    state_entries = merge_state_entries(
        manifests_dir / "l_review_batch_state.json",
        state_entries,
        args.reset_state,
    )

    batch_manifest = {
        "schema_version": "cfst-l-review-batch-manifest-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source_layout": "l-review-package",
    ...
    batch_state = {
        "schema_version": "cfst-l-review-batch-state-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
    ...
    if not args.dry_run:
        ...
        write_json(manifests_dir / "l_review_batch_manifest.json", batch_manifest)
        write_json(manifests_dir / "l_review_worker_jobs.json", worker_jobs)
        write_json(manifests_dir / "l_review_batch_state.json", batch_state)

    counts: dict[str, int] = {}
    for item in batch_entries:
        counts[item["status"]] = counts.get(item["status"], 0) + 1

    print(f"[OK] Indexed {len(batch_entries)} papers from L-review Pending root.")
    for status in sorted(counts):
        print(f"[INFO] {status}={counts[status]}")
    print(f"[INFO] Output root: {output_root}")
    if not args.dry_run:
        print(f"[OK] Batch manifest: {manifests_dir / 'l_review_batch_manifest.json'}")
        print(f"[OK] Worker jobs: {manifests_dir / 'l_review_worker_jobs.json'}")
        print(f"[OK] Batch state: {manifests_dir / 'l_review_batch_state.json'}")
    return 0
