#!/usr/bin/env python3
"""Remove one generated worker workspace after the parent no longer needs it."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Remove a generated CFST worker workspace.")
    parser.add_argument("--job-spec", type=Path, required=True, help="Path to worker_job_spec.json.")
    parser.add_argument(
        "--worker-spaces-root",
        type=Path,
        default=Path("tmp/cfst-worker-spaces"),
...
def main() -> int:
    ...
    try:
        ...
        if not isinstance(workspace_value, str) or not workspace_value:
            raise ValueError("worker_job_spec.json missing workspace.path")
